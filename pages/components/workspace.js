...
export default function Workspace({}) {
        ...
        (async () => {
          try {
            ...
            if (localStorage.getItem("host") && localStorage.getItem("port")) {
              let f = await fetch(
                          `http${
                              localStorage.getItem("host") == "localhost"
                                  ? ""
                                  : "s"}://${localStorage.getItem("host")}:${
              ...
              if (data.message === "ONLINE") {
                ...
                f = await fetch(`http${
                    localStorage.getItem("host") == "localhost" ? "" : "s"}://${
                    localStorage.getItem(
                        "host")}:${localStorage.getItem("port")}/projects`);
                data = await f.json();
  ...
  function closeProjectModal() { setNewProjectOpen(false); }
  function openProjectModal() { setNewProjectOpen(true); }
  function createProject() {
    ...
    fetch(`http${localStorage.getItem("host") == "localhost" ? "" : "s"}://${
              localStorage.getItem(
                  "host")}:${localStorage.getItem("port")}/projects/${
              gitEl.value.length > 0 ? "clone" : "new"}`,
          {
            method : "POST",
            headers : {
  ...
  async function connectServer() {
    ...
    try {
      let f =
          await fetch(`http${$("#host").value == "localhost" ? "" : "s"}://${
      ...
      if (f.message === "ONLINE") {
        ...
        f = await fetch(`http${
            $("input[placeholder='HOST']").value == "localhost"
                ? ""
                : "s"}://${$("input[placeholder='HOST']").value}:${
            $("input[placeholder='PORT']").value}/projects`);
        f = await f.json();
      ...
      window.Toast(
          "Error connecting to server", "danger",
  ...
  d = "M6.75 7.5l3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0021 18V6a2.25 2.25 0 00-2.25-2.25H5.25A2.25 2.25 0 003 6v12a2.25 2.25 0 002.25 2.25z" / >
      ...
      <div className = "mt-4 grid lg:grid-cols-2 lg:max-w-md lg:gap-4"> {
        projects.length > 0
            ? (projects.map((project, index) => {return (< ProjectCard
                    project={project}
                    onClick={
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-slate-900 p-6 text-left align-middle shadow-xl transition-all border-2 border-slate-800">
                  ...
                  >
                    ...
                    fill =
                        ...
                        line
                    x1 = {12} y1 = {19} x2 = {19} y2 = {19} className =
                        "animate-pulse" > </line>
                      </svg>
                        </div>
                  </Dialog.Title>
                        <div className = "mt-2">
                        <p className = "text-sm text-slate-300">To connect to
                            Termux or a linux server,
                    copy the following
                    ...
                    <div className="flex mt-4 bg-gray-900 border-2 items-center border-slate-700 px-4 py-2 rounded-md">
                      <p className="text-sm text-white">
                        <span className="text-blue-500">
                          curl -sL https:/ /
                                smux.rovelstars.com / install.sh|bash &&
                        smux - server</span>
                      </p>< button
                    ...
                    viewBox = "0 0 24 24"
                    strokeWidth = {1.5} stroke = "currentColor"
                    className = "w-6 h-6 ml-auto text-indigo-500" > < path
                    strokeLinecap = "round"
                    strokeLinejoin = "round"
                    d = "M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 01-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 011.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 00-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 01-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5a3.375 3.375 0 00-3.375-3.375H9.75" /
                        ...
                        <p className = "text-sm text-slate-300">Please fill in
                        the values below to connect to your server.<
                            /p>
                    <input
                    ...
                    ></input><
                        input
                    className =
                        "mt-4 bg-gray-900 rounded-md focus:outline-none border-2 border-slate-700 focus:border-indigo-700 text-slate-300 py-2 px-4"
                    ...
                    id = "connector"
                    className =
                        "text-white inline-flex text-lg justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 font-medium text-indigo-200 hover:bg-indigo-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-slate-900 p-6 text-left align-middle shadow-xl transition-all border-2 border-slate-800">
                  ...
                  <div className="mt-2">
                    ...
                    ></input><
                      ...
                      type = "button"
                      className =
                          "text-white inline-flex text-lg justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 font-medium text-indigo-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
